...
def load_templates() -> dict[str, list[str]]:
    ...
    """
    templates_path = Path(PROBES_DIR).parent / "data" / "probe_templates.json"
    if not templates_path.exists():
        print(f"ERROR: Templates not found at {templates_path}")
        sys.exit(1)
...
VINDEX = str(_REPO_ROOT / "output" / "gemma3-4b-full.vindex")
MODEL_ID = "google/gemma-3-4b-it"
TRIPLES_PATH = str(_REPO_ROOT / "knowledge" / "data" / "wikidata_triples.json")
PROBES_DIR = str(_REPO_ROOT / "knowledge" / "probes")
...
def main() -> None:
    ...
    for rel_name, templates in TEMPLATES.items():
        ...
        matched = 0
        rel_start = time.time()
        # Iterate over all template variants × all subjects
        ...
        for pi, (subject, template) in enumerate(probe_items):
            ...
            if (pi + 1) % 50 == 0:
                ...
                sys.stdout.write(
                    ...
                    f" ({matched} labels, {rate:.0f}/s, ETA {eta:.0f}s)"
                )
                sys.stdout.flush()
        ...
        print(
            ...
            f" x {nvariants} templates -> {matched:3d} features"
            f"  ({rate:.1f} probes/s)"
        )
    ...
    data_path = Path(PROBES_DIR) / "gemma-3-4b-it" / "feature_labels.json"
